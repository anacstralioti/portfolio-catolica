...
# Tela de introdução de fase, exibe "Capítulo N, Nome da Fase" com fade.
# Lê os dados de GameGlobal para saber qual fase apresentar,
# depois redireciona para a cena correta conforme next_phase_number.
...
func _ready() -> void:
	...
	# Sequência: invisível → fade-in (1.5s) → pausa (3s) → fade-out (1.5s) → troca de cena
	modulate.a = 0.0
	var tw := create_tween()
...
func _transition() -> void:
	# Decide qual cena carregar com base no número de fase armazenado no GameGlobal
	match GameGlobal.next_phase_number:
		...
		_:
			# Fase desconhecida → volta ao menu principal como fallback seguro
