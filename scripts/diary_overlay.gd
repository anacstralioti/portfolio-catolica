...
@onready var stain_a:   ColorRect = $Root/WaterStainA
@onready var stain_b:   ColorRect = $Root/WaterStainB
@onready var title_lbl: Label     = $Root/Title
@onready var read1:     Label     = $Root/ReadLine1
...
func _layout() -> void:
	...
	var pad_y := ph * 0.05
	var iw    := pw - pad_x * 2.0
	var fs    := maxi(8, int(vp.x / 40))
	var lh    := float(fs) * 1.35
	...
	close_hint.position = Vector2(px, py + ph + 3.0)
	close_hint.size     = Vector2(pw, float(hint_fs) * 1.6)
