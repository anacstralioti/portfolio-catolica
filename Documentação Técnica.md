...
**Autora**: Ana Carolina Fanhani Stralioti
...
7. [Avaliação dos Requisitos do Projeto](#7-avaliação-dos-requisitos-do-projeto)
8. [Análise de Engenharia de Software](#8-análise-de-engenharia-de-software)
9. [DevOps e Infraestrutura — Estado Atual e Recomendações](#9-devops-e-infraestrutura--estado-atual-e-recomendações)
...
## 7. Avaliação dos Requisitos do Projeto

### Requisitos Funcionais

| Requisito | Implementação | Arquivo(s) |
|---|---|---|
| RF01 — Controle do personagem | `CharacterBody2D` com gravidade, walk/run/jump/crouch | [player.gd](scripts/player.gd) |
| RF02 — Sistema de coleta | `CollectArea`, sinais, HUD com slots | [player.gd](scripts/player.gd), [hud.gd](scripts/hud.gd) |
| RF03 — FSM ambiental | `enum State`, `_process()` com transições, clima procedural | [game_manager.gd](scripts/game_manager.gd) |
| RF04 — Checkpoints | `Area2D` → `register_checkpoint()` → `_save()` | [checkpoint.gd](scripts/checkpoint.gd), [game_manager.gd](scripts/game_manager.gd) |
| RF05 — Interações com itens | SandHole (pá), SandCastle (balde), MemoryObject | [sand_hole.gd](scripts/sand_hole.gd), [sand_castle.gd](scripts/sand_castle.gd) |
| RF06 — HUD e interface | Barra de inventário, barra de progresso, narrativa | [hud.gd](scripts/hud.gd), [narrative_ui.gd](scripts/narrative_ui.gd) |
| RF07 — Narrativa dinâmica | Textos por estado FSM + dicionário de narrativas | [game_manager.gd](scripts/game_manager.gd) |

### Requisitos Não-Funcionais

| Requisito | Status | Evidência |
|---|---|---|
| RNF01 — Usabilidade | ✅ Implementado | Prompts contextuais, menus navegáveis por teclado e mouse |
| RNF02 — Portabilidade (Windows) | ✅ Configurado | Export template no Godot Editor; executável .exe gerado |
| RNF03 — Segurança de dados | ✅ Implementado | Validação de JSON no load; nenhuma coleta de dados externos |
| RNF04 — Acessibilidade | ✅ Implementado | Contraste alto na UI, fontes legíveis, textos narrativos na tela |
| RNF05 — Armazenamento local | ✅ Implementado | `user://fase1_save_slot{N}.json` via `FileAccess` |

---

## 8. Análise de Engenharia de Software

### Princípios SOLID Aplicados

**Single Responsibility (SRP)**  
Cada script tem uma única razão para mudar:
- `game_manager.gd` → FSM e estado do jogo
- `player.gd` → física e input do personagem
- `hud.gd` → apresentação visual do inventário
- `narrative_ui.gd` → exibição de texto com fila

**Open/Closed (OCP)**  
`memory_object.gd` é aberto para extensão via `@export`, fechado para modificação. Novos tipos de objeto de memória são configurados no Editor, não no código.

**Liskov Substitution (LSP)**  
Todos os NPCs (`crab.gd`, `seagull.gd`, `kite.gd`) podem ser adicionados à cena independentemente — nenhum depende do tipo concreto de outro.

**Interface Segregation (ISP)**  
Comunicação via grupos Godot (`"player"`, `"game_manager"`, `"hud"`) e `has_method()`:
```gdscript
if gm and gm.has_method("show_custom_narrative"):
    gm.show_custom_narrative(text)
```
Os objetos não precisam de uma referência tipada ao `GameManager`.

**Dependency Inversion (DIP)**  
Módulos dependem de abstrações (grupos, sinais, métodos por nome), não de implementações concretas.

### Design Patterns Identificados

| Padrão | Implementação |
|---|---|
| **State** | `enum State` em `game_manager.gd` com `_enter()` e `_process()` |
| **Observer** | Sinais Godot: `shell_collected`, `item_picked_up`, `interact_pressed`, `phase_complete` |
| **Singleton** | `GameGlobal` via Autoload |
| **Template Method** | Ciclo `_ready()` → `_process()` → `_use()` nos objetos de memória |
| **Command** | Sinal `interact_pressed` como intenção descoberta do objeto |
| **Strategy** | Overlays intercambiáveis (`photo`, `diary`, `flashback`) |
| **Factory** | Carregamento dinâmico: `load("res://scenes/flashback_overlay.tscn").instantiate()` |

---

## 9. DevOps e Infraestrutura — Estado Atual e Recomendações

### O que já existe

| Item | Status |
|---|---|
| Controle de versão (Git + GitHub) | ✅ Ativo |
| Deploy em itch.io (Windows .exe) | ✅ Hospedado no itch.io |
| Exportação Windows (.exe) | ✅ Configurada no Godot |
| Documentação no GitHub | ✅ README.md, GDD.md, este arquivo |

### O que ainda precisa ser implementado

#### CI/CD com GitHub Actions

Criar `.github/workflows/export.yml` para exportar o jogo automaticamente a cada push na branch `main`:

```yaml
# Exemplo de pipeline — adapte com a versão correta do Godot
  ...
  export-windows:
    ...
    steps:
      ...
      - name: Export (Windows)
        ...
        with:
          godot_executable_download_url: "https://downloads.tuxfamily.org/godotengine/4.5/..."
...
#### Ambientes Distintos (Dev / Staging / Prod)

Usar branches do Git para separar ambientes:
- `dev` → desenvolvimento (build automático para testes internos)
- `staging` → versão para revisores do PAC
- `main` → produção (itch.io)

#### Observabilidade

Para um jogo, observabilidade significa entender como os jogadores interagem. Implementações possíveis:
- **Log local**: registrar eventos em `user://session_log.json` (estados visitados, tempo por zona)
- **Telemetria simples**: ao final da fase, salvar `{"playtime": X, "deaths": Y, "items": [...]}` no save
- **Godot Profiler**: já integrado ao editor para performance em tempo de desenvolvimento

#### Contribuições Externas

Para viabilizar contribuições, adicionar ao repositório:
- `CONTRIBUTING.md` — como configurar o ambiente, padrões de código, como abrir issues
...
- [Godot Engine 4.5](https://godotengine.org/download) (mesma versão do projeto)
- Git
...
feature/*  ← funcionalidades em progresso
fix/*      ← correções de bugs
```

### Padrões de Código

- Funções públicas: `snake_case` sem underscore inicial
- Funções privadas: `_snake_case` com underscore inicial
- Constantes: `UPPER_SNAKE_CASE`
- Sinais: `snake_case` no tempo passado ou presente (`shell_collected`, `phase_complete`)
- Grupos Godot: string em minúsculas (`"player"`, `"game_manager"`)

### Reportando Bugs

Abra uma Issue no GitHub com:
1. Versão do Godot utilizada
